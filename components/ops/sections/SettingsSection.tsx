      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Panel title="운영 규칙 / spec">
          <div className="space-y-3">
            ...
            <ChecklistRow item={{ id: "settings-7", label: "Direct assistant -> Supabase ingest path", status: data.dataSource.sourceHealth.supabaseReachable ? "done" : (data.dataSource.sourceHealth.supabaseConfigured ? "blocked" : "doing"), note: "`POST /api/ops/ingest` can write worklog + artifact rows without Mac mini watcher" }} />
            <ChecklistRow item={{ id: "settings-8", label: "Mac mini automation watcher status visibility", status: data.dataSource.sourceHealth.automation?.mode && data.dataSource.sourceHealth.automation.mode !== "manual" ? "done" : "doing", note: `${data.dataSource.sourceHealth.automation?.mode || "manual"} / ${data.dataSource.sourceHealth.automation?.state || "manual"}` }} />
            <ChecklistRow item={{ id: "settings-9", label: "서버 기반 인증으로 전환", status: "todo", note: "현재는 hardcoded access code MVP 보호" }} />
          </div>
        </Panel>
        <div className="space-y-6">
          <Panel title="Source metadata">
            <div className="grid gap-4 text-sm text-white/75 md:grid-cols-2">
              ...
              <InfoTile label="Typed artifacts" value={`${data.dataSource.sourceHealth.artifactsCount ?? data.artifacts.length}`} />
              <InfoTile label="Decisions / Learnings" value={`${data.dataSource.sourceHealth.decisionsCount ?? data.artifacts.filter((artifact) => artifact.artifactType === "decision").length} / ${data.dataSource.sourceHealth.learningsCount ?? data.artifacts.filter((artifact) => artifact.artifactType === "learning").length}`} />
              <InfoTile label="Artifacts updated at" value={formatDateTime(data.dataSource.sourceHealth.artifactsUpdatedAt || data.artifacts[0]?.updatedAt || data.dataSource.sourceHealth.worklogsUpdatedAt || data.worklogs[0]?.updatedAt)} />
